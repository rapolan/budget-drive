...
**Note:** the `[0.5.7]` entry below describes `home_zip_code`/`service_zip_codes` columns and a migration (`039_add_instructor_location_fields.sql`) that were never actually applied to this codebase - that work was aspirational and disconnected (frontend-only free-text fields with no backing migration or column; the service silently discarded them on every save). This release replaces it with an actual, migration-backed `instructor_service_areas` table.

**Update (health-audit fix-up, Pass 3):** this entry originally described a **filter-with-fallback** design ("prefers in-area, falls back to every candidate if zero results") - that design was found broken shortly after shipping (an instructor with no configured service area always counts as in-area, so the "zero results" fallback essentially never fired, meaning a configured instructor could be silently excluded from every search even when they were the closest match) and was replaced with the current **rank-only, never-exclude** design before this entry was corrected: `outsideServiceArea` is purely a sort key (confirmed live in `schedulingService.ts` - it decides ordering only, at the comparator that sorts in-area results before out-of-area ones), never a filter. Every candidate instructor's slots are always returned; area membership only affects sort position. See `docs/BLUEPRINTS.md`'s "Instructor Licensing and Service Areas" section for the full story of why the filter-with-fallback design was abandoned.
...
- The ranked slot search (`findRankedAvailableSlots`) always returns every candidate instructor's slots; in-service-area instructors sort above out-of-area ones (`outsideServiceArea`, a display/ranking flag only), never filtered out.
